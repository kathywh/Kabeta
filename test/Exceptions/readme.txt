...
02_1:  IF-Stage Fault in branch slots (User Mode)           Fetch one instruction of Invalid I-Address
                                                            fault (8000_0014), then go to target address
...
03_4:  Interrupt when branch at MA-Stage                    Go to Int 0 Handler (8000_0018), return to 81C (br target)
...
05:    Interrupt and pipeline stall (EX-Stage)				Go to Int 0 Handler (8000_0018), return and no stall
06_1:  Branch (BEQ) and pipeline stall (EX-Stage)			Stall, then branch
06_2:  Branch (JMP) and pipeline stall (EX-Stage)			Stall, then branch
07:    Double fault                                         Illegal Instruction fault (8000_0008), then
                                                            double fault, system reset